...
export const ThankYou2 = (): JSX.Element => {
  const whatsappNumber = "5594992508057";
  const whatsappLink = `https://wa.me/${whatsappNumber}`;

  const [isDarkMode, setIsDarkMode] = useState(false);

  // Push virtual pageview to GTM dataLayer on mount (qualified lead page)
  useEffect(() => {
    if (typeof window !== 'undefined') {
      (window as any).dataLayer = (window as any).dataLayer || [];
      (window as any).dataLayer.push({
        event: 'virtualPageview',
        virtualPageURL: '/obrigado2',
        virtualPageTitle: 'Obrigado Lead Qualificado - Grupo Rugido',
        page_path: '/obrigado2',
        page_title: 'Obrigado Lead Qualificado - Grupo Rugido'
      });
      console.log("GTM dataLayer push: virtualPageview /obrigado2 (qualified lead)");
    }
  }, []);
